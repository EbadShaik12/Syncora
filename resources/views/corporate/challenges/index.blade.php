@extends('layouts.app')
@section('title', 'My Innovation Challenges')
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10 animate-fade-in">
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between mb-10 gap-6">
        ...
        <a href="{{ route('corporate.challenges.create') }}" class="magnetic shimmer-btn bg-gradient-to-r from-purple-600 to-pink-600 text-white px-6 py-3.5 rounded-xl font-bold shadow-lg hover:scale-105 transition flex items-center gap-2">
    ...
    @if($challenges->isEmpty())
        <div class="glass-card-strong rounded-[2.5rem] p-16 text-center border-glow max-w-2xl mx-auto">
            ...
            <a href="{{ route('corporate.challenges.create') }}" class="inline-flex items-center gap-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white px-8 py-3.5 rounded-xl font-bold shadow-xl shadow-purple-500/20 hover:scale-105 transition">
            ...
            @foreach($challenges as $i => $challenge)
                <div class="reveal glass-card-strong rounded-3xl p-6 border-glow flex flex-col hover:scale-[1.02] transition-all duration-300 relative group"
                    ...
                    <div class="flex justify-between items-start mb-6">
                        <span class="text-xs font-bold uppercase tracking-wider px-3.5 py-1.5 rounded-xl shadow-sm {{ $challenge->statusColor() }}">
                    ...
                    <div class="border-t border-gray-100 dark:border-gray-800/50 pt-4 mb-6 space-y-2 text-xs font-semibold text-gray-500">
                        <div class="flex justify-between">
                            ...
                            <span class="text-purple-600 font-bold">₹{{ number_format($challenge->budget_min) }} – ₹{{ number_format($challenge->budget_max) }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span>Deadline:</span>
                            <span class="text-gray-900 dark:text-white font-bold">{{ $challenge->deadline->format('M d, Y') }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span>Submissions:</span>
                            <span class="text-gray-900 dark:text-white font-black bg-gray-100 dark:bg-gray-800 px-2.5 py-0.5 rounded-lg">{{ $challenge->applications_count }}</span>
                        </div>
                    </div>

                    <div class="grid grid-cols-2 gap-3 mt-auto">
                        <a href="{{ route('corporate.challenges.applications', $challenge) }}" class="px-4 py-3 bg-purple-50 dark:bg-purple-900/30 border border-purple-100 dark:border-purple-800/50 text-purple-600 dark:text-purple-400 font-bold rounded-xl text-center text-xs hover:bg-purple-100 transition shadow-sm">
                            👥 View Proposals
                        </a>
                        <a href="{{ route('corporate.challenges.kanban', $challenge) }}" class="px-4 py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold rounded-xl text-center text-xs hover:scale-105 transition shadow-md shadow-purple-500/20">
                            📋 Kanban Flow
                        </a>
                    </div>
                    
                    {{-- Secondary Actions overlay on hover --}}
                    <div class="flex gap-2 justify-end mt-4 pt-3 border-t border-gray-100 dark:border-gray-800/50">
                        <a href="{{ route('corporate.challenges.edit', $challenge) }}" class="text-xs font-bold text-gray-500 hover:text-blue-500 transition flex items-center gap-1 px-2.5 py-1.5 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20">
                            ✏️ Edit
                        </a>
                        <form action="{{ route('corporate.challenges.destroy', $challenge) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this challenge?')">
                            @csrf
                            @method('DELETE')
