    ...
    <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-3xl border border-slate-100 dark:border-zinc-800 p-5 hover:shadow-xl transition flex flex-col relative overflow-hidden group">
            ...
            <div class="flex items-center gap-3 mb-4">
                <img src="{{ $challenge->corporate->logoUrl() }}" class="w-11 h-11 rounded-full object-cover shadow-sm border border-slate-200 dark:border-zinc-700">
                <div class="flex-grow min-w-0">
                    <div class="flex items-center justify-between">
                        <span class="font-extrabold text-sm text-slate-800 dark:text-white truncate">{{ $challenge->corporate->companyName() }}</span>
                    ...
                    <p class="text-[10px] text-gray-500 dark:text-zinc-400 font-semibold flex items-center gap-1 mt-0.5">
                        ...
                        <span>{{ $challenge->created_at ? $challenge->created_at->diffForHumans() : 'Recently' }}</span>
                        <span class="text-gray-400 dark:text-zinc-600">•</span>
                        <span>🌎</span>
                    </p>
                </div>
            ...
            {{-- Description --}}
            <p class="text-xs text-slate-500 dark:text-zinc-350 leading-relaxed font-medium line-clamp-3 mb-4">{{ $challenge->description }}</p>
                ...
                <div class="mb-4">
                    @if($isImage)
                        <div class="border border-slate-100 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-inner bg-slate-50 dark:bg-zinc-900/40 relative group cursor-pointer"
                             @click="lightboxOpen = true; lightboxImage = '{{ asset('storage/' . $challenge->attachment_path) }}'; lightboxTitle = '{{ $challenge->title }}'">
                            <img src="{{ asset('storage/' . $challenge->attachment_path) }}" class="w-full h-44 object-cover group-hover:scale-[1.01] transition-transform duration-500">
                        ...
                        <div class="border border-slate-200 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm bg-slate-50 dark:bg-zinc-900/40">
                            <div class="bg-slate-100 dark:bg-zinc-850 px-3.5 py-2 flex items-center justify-between border-b border-slate-200 dark:border-zinc-850">
                                <div class="flex items-center gap-1.5 min-w-0">
                                    ...
                                    <span class="text-xs font-bold text-slate-700 dark:text-zinc-300 truncate">{{ $challenge->attachment_filename }}</span>
                                </div>
                                <a href="{{ asset('storage/' . $challenge->attachment_path) }}" target="_blank" class="px-2.5 py-1 bg-white hover:bg-slate-50 dark:bg-zinc-800 dark:hover:bg-zinc-750 text-slate-800 dark:text-white rounded-lg border border-slate-200/80 dark:border-zinc-700 text-[10px] font-bold flex items-center gap-1 shadow-sm transition">
                            ...
                            <iframe src="{{ asset('storage/' . $challenge->attachment_path) }}#toolbar=0" class="w-full h-48 border-0 bg-white dark:bg-zinc-900"></iframe>
                        </div>
                    @elseif($isPpt)
                        <div class="border border-orange-500/20 rounded-2xl overflow-hidden shadow-sm bg-[#c43e1c]/5 dark:bg-[#c43e1c]/10 p-4 flex flex-col justify-between h-44 border-l-4 border-l-orange-500">
                            <div class="flex items-start gap-3">
                                ...
                                <div class="min-w-0">
                                    <h4 class="font-extrabold text-xs text-slate-800 dark:text-white truncate">{{ $challenge->attachment_filename }}</h4>
                            ...
                            <div class="grid grid-cols-2 gap-2 mt-auto">
                                <a href="{{ asset('storage/' . $challenge->attachment_path) }}" target="_blank" class="py-2 text-center text-[10px] font-black bg-orange-600 hover:bg-orange-700 text-white rounded-lg shadow-sm transition hover:scale-[1.02]">
                                    Download
                                </a>
                                <a href="https://view.officeapps.live.com/op/embed.aspx?src={{ urlencode(secure_url('storage/' . $challenge->attachment_path)) }}" target="_blank" class="py-2 text-center text-[10px] font-black bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-slate-700 dark:text-zinc-300 rounded-lg hover:bg-slate-50 transition hover:scale-[1.02]">
                                    View slides
                                </a>
                            </div>
                        </div>
                    @elseif($isWord)
                        <div class="border border-blue-500/20 rounded-2xl overflow-hidden shadow-sm bg-[#185abd]/5 dark:bg-[#185abd]/10 p-4 flex flex-col justify-between h-44 border-l-4 border-l-blue-600">
                            ...
                            <div class="grid grid-cols-2 gap-2 mt-auto">
                                <a href="{{ asset('storage/' . $challenge->attachment_path) }}" target="_blank" class="py-2 text-center text-[10px] font-black bg-blue-600 hover:bg-blue-750 text-white rounded-lg shadow-sm transition hover:scale-[1.02]">
                                    Download
                                </a>
                                <a href="https://view.officeapps.live.com/op/embed.aspx?src={{ urlencode(secure_url('storage/' . $challenge->attachment_path)) }}" target="_blank" class="py-2 text-center text-[10px] font-black bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-slate-700 dark:text-zinc-300 rounded-lg hover:bg-slate-50 transition hover:scale-[1.02]">
                                    View doc
                    ...
                    @else
                        <div class="border border-slate-200 dark:border-zinc-800 rounded-xl overflow-hidden shadow-sm bg-slate-50 dark:bg-zinc-900/40 p-3 flex items-center justify-between">
                            <div class="flex items-center gap-2.5 min-w-0">
                                ...
                                <span class="text-xs font-bold text-slate-750 dark:text-zinc-300 truncate">{{ $challenge->attachment_filename }}</span>
                            </div>
                            <a href="{{ asset('storage/' . $challenge->attachment_path) }}" target="_blank" class="px-3 py-1.5 bg-purple-600 text-white text-[10px] font-bold rounded-lg hover:bg-purple-700 transition flex items-center gap-1 shadow-sm">
                                Download
                            </a>
                        </div>
                    @endif
            ...
            <div class="border-t border-slate-100 dark:border-zinc-800/80 pt-4 mt-auto space-y-2 text-xs font-bold text-slate-500">
                <div class="flex justify-between items-center">
                    ...
                    <span class="text-primary-600 font-extrabold">₹{{ number_format($challenge->budget_min) }} – ₹{{ number_format($challenge->budget_max) }}</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-gray-400 dark:text-zinc-500">Deadline:</span>
                    <span class="text-slate-800 dark:text-zinc-300">{{ $challenge->deadline->format('M d, Y') }} <span class="text-gray-400 dark:text-zinc-500 font-medium">({{ $challenge->deadline->diffForHumans() }})</span></span>
                </div>
            </div>
            ...
            @else
                <a href="{{ route('startup.challenges.apply', $challenge) }}" class="mt-4 w-full py-3.5 rounded-xl bg-primary-600 hover:bg-primary-750 text-white font-extrabold text-xs text-center block transition hover:scale-[1.01] shadow-md shadow-primary-500/10">Apply Now</a>
            @endif
        </div>
        @endforeach
    </div>
    <div class="mt-8">{{ $challenges->withQueryString()->links() }}</div>
    @endif
