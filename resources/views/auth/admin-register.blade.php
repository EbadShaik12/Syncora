@extends('layouts.app')
@section('title', 'Register Admin Account')
...
<div class="min-h-screen flex items-center justify-center relative overflow-hidden bg-transparent px-4 py-12">
    ...
    <div class="w-full max-w-md relative stagger-container">
        ...
        <div class="reveal text-center mb-8">
            <a href="{{ route('home') }}" class="inline-flex flex-col items-center gap-4 mb-2 group">
        ...
        <div class="reveal reveal-delay-2 rounded-3xl p-8 shadow-2xl relative overflow-hidden border border-amber-500/20"
            ...
            <form method="POST" action="{{ route('admin.register.post') }}" class="space-y-5">
                ...
                <div class="input-group">
                    <input type="text" name="name" id="admin_name" value="{{ old('name') }}" required autofocus placeholder=" "
                    ...
                    @error('name')<p class="text-red-400 text-xs mt-1 font-semibold">{{ $message }}</p>@enderror
                </div>

                {{-- Email --}}
                <div class="input-group">
                    <input type="email" name="email" id="admin_email" value="{{ old('email') }}" required placeholder=" "
                           class="w-full rounded-xl border border-gray-700/50 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/30 outline-none transition text-white bg-black/20">
                    <label for="admin_email">Email Address</label>
                    @error('email')<p class="text-red-400 text-xs mt-1 font-semibold">{{ $message }}</p>@enderror
                </div>
                ...
                <div class="input-group">
                    ...
                    @error('password')<p class="text-red-400 text-xs mt-1 font-semibold">{{ $message }}</p>@enderror
                </div>
                ...
                <div class="input-group">
                    ...
                    @error('admin_secret_key')<p class="text-red-400 text-xs mt-1 font-semibold">{{ $message }}</p>@enderror
                </div>
            ...
            <div class="mt-6 pt-6 border-t border-white/5 text-center">
                <p class="text-gray-500 text-sm">Already have an admin account?
                    <a href="{{ route('admin.login') }}" class="text-amber-400 font-bold hover:text-amber-300 transition">Admin Login →</a>
                </p>
            </div>
        </div>
    </div>
