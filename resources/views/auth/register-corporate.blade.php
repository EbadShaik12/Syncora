@extends('layouts.app')
@section('title', 'Register as Corporate')
...
<div class="min-h-screen flex items-center justify-center px-4 py-12 relative overflow-hidden bg-transparent">
    ...
    <div class="w-full max-w-4xl relative z-10 stagger-container grid lg:grid-cols-5 gap-8 items-start mt-8">
        ...
        <div class="lg:col-span-2 lg:sticky lg:top-24 text-center lg:text-left reveal">
            <a href="{{ route('register') }}" class="inline-flex items-center gap-2 text-sm font-semibold text-gray-500 dark:text-gray-400 hover:text-purple-600 transition mb-8 magnetic">
        ...
        <div class="lg:col-span-3 reveal reveal-delay-2 glass-card-strong rounded-3xl p-8 shadow-2xl border-glow">
            <form method="POST" action="{{ route('register.corporate') }}">
                @csrf
                ...
                <div class="relative">
                    ...
                    <div class="grid sm:grid-cols-2 gap-4 mt-2">
                        ...
                        <div class="input-group">
                            <select name="company_size" id="company_size" required class="w-full rounded-xl border border-gray-200 dark:border-gray-700/50 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/30 outline-none transition text-gray-900 dark:text-white appearance-none">
                                <option value="small" {{ old('company_size') == 'small' ? 'selected' : '' }}>Small (1-50)</option>
                                <option value="medium" {{ old('company_size', 'medium') == 'medium' ? 'selected' : '' }}>Medium (51-500)</option>
                                <option value="large" {{ old('company_size') == 'large' ? 'selected' : '' }}>Large (501-5000)</option>
                                <option value="enterprise" {{ old('company_size') == 'enterprise' ? 'selected' : '' }}>Enterprise (5000+)</option>
                            </select>
                    ...
                    <div class="input-group mt-2">
                        <textarea name="problem_statement" id="problem_statement" rows="3" required minlength="20" maxlength="1000" placeholder=" "
                            class="w-full rounded-xl border border-gray-200 dark:border-gray-700/50 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/30 outline-none transition resize-none text-gray-900 dark:text-white">{{ old('problem_statement') }}</textarea>
                        <label for="problem_statement">What are you looking for? (Problem Statement)</label>
                        @error('problem_statement')<p class="text-red-500 text-xs mt-1 absolute -bottom-5">{{ $message }}</p>@enderror
                    </div>
