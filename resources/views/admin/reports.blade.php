@extends('layouts.app')
@section('title', 'Platform Reports & Exports — Admin')
@section('content')
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.reports.export.users') }}" class="px-4 py-2.5 bg-primary-600 hover:bg-primary-700 text-white rounded-xl font-bold text-sm shadow-md shadow-primary-500/20 hover:scale-105 transition flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                </svg>
                Export Users CSV
            </a>
            <a href="{{ route('admin.reports.export.connections') }}" class="px-4 py-2.5 bg-slate-900 hover:bg-slate-800 dark:bg-white dark:hover:bg-slate-100 text-white dark:text-slate-950 rounded-xl font-bold text-sm shadow-md hover:scale-105 transition flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                </svg>
                Export Matches CSV
    ...
    <div class="mb-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div class="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 rounded-2xl p-5 hover:shadow-md transition">
                ...
                <p class="text-3xl font-black text-slate-800 dark:text-white mt-1">{{ $health['db_users'] }}</p>
                <div class="flex items-center gap-1.5 mt-2">
                    <span class="text-[10px] px-1.5 py-0.5 rounded bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400 font-bold">
                        {{ $health['pending_users'] }} pending approval
                    </span>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 rounded-2xl p-5 hover:shadow-md transition">
                ...
                <p class="text-3xl font-black text-slate-800 dark:text-white mt-1">{{ $health['db_connections'] }}</p>
                <div class="flex items-center gap-1.5 mt-2">
            ...
            <div class="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 rounded-2xl p-5 hover:shadow-md transition">
                ...
                <p class="text-3xl font-black text-slate-800 dark:text-white mt-1">{{ $health['db_messages'] }}</p>
                <div class="flex items-center gap-1.5 mt-2">
            ...
            <div class="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 rounded-2xl p-5 hover:shadow-md transition">
                ...
                <p class="text-3xl font-black text-slate-800 dark:text-white mt-1">{{ $health['db_signals'] }}</p>
                <div class="flex items-center gap-1.5 mt-2">
    ...
    <div class="grid md:grid-cols-3 gap-6 mb-8">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-zinc-800">
                        @foreach($growth as $g)
                        <tr>
                            <td class="py-3.5 font-bold text-slate-900 dark:text-white">{{ $g['month'] }}</td>
        ...
        {{-- Platform Pipeline Health Status --}}
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex-1">
                        <div class="flex justify-between items-center text-xs font-bold text-slate-500">
                            ...
                            <span class="text-slate-800 dark:text-white">{{ $health['open_challenges'] }}</span>
                        </div>
                        <div class="h-2 bg-slate-100 dark:bg-zinc-800 rounded-full mt-1.5 overflow-hidden">
                            <div class="h-full bg-blue-500 rounded-full" style="width: {{ min(100, max(20, $health['open_challenges'] * 5)) }}%"></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex-1">
                        <div class="flex justify-between items-center text-xs font-bold text-slate-500">
                            ...
                            <span class="text-slate-800 dark:text-white">{{ $health['pending_apps'] }}</span>
                        </div>
                        <div class="h-2 bg-slate-100 dark:bg-zinc-800 rounded-full mt-1.5 overflow-hidden">
                            <div class="h-full bg-purple-500 rounded-full" style="width: {{ min(100, max(20, $health['pending_apps'] * 8)) }}%"></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex-1">
                        <div class="flex justify-between items-center text-xs font-bold text-slate-500">
                            ...
                            <span class="text-slate-800 dark:text-white">{{ $health['pending_users'] }}</span>
                        </div>
                        <div class="h-2 bg-slate-100 dark:bg-zinc-800 rounded-full mt-1.5 overflow-hidden">
                            <div class="h-full bg-yellow-500 rounded-full" style="width: {{ min(100, max(20, $health['pending_users'] * 10)) }}%"></div>
                        </div>
                    </div>
                </div>
