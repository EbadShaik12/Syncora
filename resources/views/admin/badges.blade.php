@extends('layouts.app')
@section('title', 'Badges & Gamification — Admin')
@section('content')
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex items-center justify-between mb-8">
        ...
        <span class="px-3 py-1.5 rounded-full bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300 text-xs font-black">{{ $badges->count() }} Badges</span>
    </div>

    @if(session('success'))
        <div class="mb-4 px-4 py-3 bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 rounded-xl text-sm font-medium border border-green-200">✅ {{ session('success') }}</div>
    @endif
    @if(session('error'))
        <div class="mb-4 px-4 py-3 bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300 rounded-xl text-sm font-medium border border-red-200">❌ {{ session('error') }}</div>
    @endif
    ...
    <div class="grid lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-6">
            ...
            <form method="POST" action="{{ route('admin.badges.store') }}" class="space-y-4">
                @csrf
                <div>
                    <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Badge Name *</label>
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="e.g. First Connection" required
                ...
                <div>
                    ...
                    <textarea name="description" rows="2" placeholder="What is this badge for?" required
                              class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 focus:border-yellow-400 outline-none transition text-sm resize-none">{{ old('description') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="criteria_value" value="{{ old('criteria_value') }}" placeholder="e.g. 10"
            ...
            <div x-data="{ showAward: false }" class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-5">
                <div class="flex items-start justify-between">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h3 class="font-black text-lg text-gray-900 dark:text-white">{{ $badge->name }}</h3>
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ $badge->description }}</p>
                            <div class="flex items-center gap-2 mt-1.5">
                                <span class="px-2 py-0.5 rounded-full bg-orange-50 dark:bg-orange-900/30 text-orange-600 text-xs font-bold">{{ $badge->criteria_type }}</span>
                                @if($badge->criteria_value)
                                    <span class="text-xs text-gray-400">≥ {{ $badge->criteria_value }}</span>
                                @endif
                                <span class="px-2 py-0.5 rounded-full bg-yellow-50 dark:bg-yellow-900/30 text-yellow-700 text-xs font-bold">{{ $badge->users_count }} awarded</span>
                            </div>
                        </div>
                    </div>
                ...
                <div x-show="showAward" x-cloak class="mt-4 pt-4 border-t border-gray-100 dark:border-gray-800">
                    ...
                    <form method="POST" action="{{ route('admin.badges.award', $badge) }}" class="flex gap-2">
                        @csrf
                        <select name="user_id" required class="flex-1 px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-sm outline-none focus:border-yellow-400">
                            ...
                            @foreach($users as $u)
                                <option value="{{ $u->id }}">{{ $u->companyName() }} ({{ $u->role }})</option>
                            @endforeach
                        </select>
                ...
                <div class="mt-3 flex items-center gap-2">
                    ...
                    @foreach($badge->users->take(6) as $holder)
                    <div class="relative group">
                        <img src="{{ $holder->logoUrl() }}" class="w-7 h-7 rounded-lg border-2 border-white dark:border-gray-900 shadow" data-tooltip="{{ $holder->companyName() }}">
                    </div>
                    @endforeach
                    @if($badge->users_count > 6)
                        <span class="text-xs text-gray-400">+{{ $badge->users_count - 6 }} more</span>
                    @endif
                </div>
                @endif
            </div>
