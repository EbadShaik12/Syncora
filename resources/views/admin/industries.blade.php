@extends('layouts.app')
@section('title', 'Industry Management — Admin')
@section('content')
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex items-center justify-between mb-8">
        ...
        <div class="flex items-center gap-3">
            <span class="px-3 py-1.5 rounded-full bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 text-xs font-black">{{ $industries->count() }} Industries</span>
        </div>
    </div>

    @if(session('success'))
        <div class="mb-4 px-4 py-3 bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 rounded-xl text-sm font-medium border border-green-200 dark:border-green-800/40">✅ {{ session('success') }}</div>
    @endif
    @if(session('error'))
        <div class="mb-4 px-4 py-3 bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300 rounded-xl text-sm font-medium border border-red-200 dark:border-red-800/40">❌ {{ session('error') }}</div>
    @endif
        ...
        <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-6">
            ...
            <form method="POST" action="{{ route('admin.industries.store') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="e.g. HealthTech"
                ...
                <div x-data="{ editing: false }" class="px-6 py-4 hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition">
                    <div x-show="!editing" class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="font-bold text-gray-900 dark:text-white">{{ $industry->name }}</p>
                                <p class="text-xs text-gray-400">{{ $industry->startup_profiles_count + $industry->corporate_profiles_count }} profiles using this</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-2">
                            <span class="px-2 py-0.5 rounded-full bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 text-xs font-bold">{{ $industry->startup_profiles_count }} startups</span>
                            <span class="px-2 py-0.5 rounded-full bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 text-xs font-bold">{{ $industry->corporate_profiles_count }} corporates</span>
                            ...
                            <form method="POST" action="{{ route('admin.industries.destroy', $industry) }}" onsubmit="return confirm('Delete {{ $industry->name }}?')">
                                @csrf @method('DELETE')
                    ...
                    <div x-show="editing" x-cloak>
                        <form method="POST" action="{{ route('admin.industries.update', $industry) }}" class="flex gap-3">
                            @csrf @method('PUT')
                            <input type="text" name="icon" value="{{ $industry->icon }}" class="w-16 px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-sm outline-none" placeholder="🏭">
                            <input type="text" name="name" value="{{ $industry->name }}" class="flex-1 px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-sm outline-none focus:border-primary-500" required>
