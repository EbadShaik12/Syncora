...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex items-center justify-between mb-8">
        ...
        <div class="flex items-center gap-2">
            ...
            <a href="{{ route('admin.users.index') }}" class="px-4 py-2 text-sm font-semibold bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-xl transition">Manage Users →</a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <div class="stat-card section-card p-5 hover:shadow-lg transition group cursor-default">
            <div class="flex items-start justify-between mb-3">
                <div class="w-10 h-10 rounded-xl bg-gradient-to-br {{ $kpi['gradient'] }} flex items-center justify-center text-white shadow-md group-hover:scale-110 transition">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $kpi['icon'] }}"/>
                    </svg>
                </div>
            </div>
            <p class="text-3xl font-black mb-0.5">{{ $kpi['value'] }}</p>
            <p class="text-sm font-semibold text-gray-700 dark:text-gray-300">{{ $kpi['label'] }}</p>
            <p class="text-[10px] text-gray-400 mt-0.5">{{ $kpi['delta'] }}</p>
        </div>
        @endforeach
    </div>

    {{-- ── Row 1: Activity Timeline + User Doughnut ── --}}
        ...
        <div class="chart-card section-card p-6 flex flex-col">
            ...
            <div class="grid grid-cols-2 gap-2 mt-4">
                ...
                <div class="flex items-center gap-1.5">
                    <span class="w-2.5 h-2.5 rounded-full {{ $statusColors[$key] }} flex-shrink-0"></span>
                    <span class="text-[10px] text-gray-500 capitalize">{{ $key }}</span>
                    <span class="text-[10px] font-bold ml-auto">{{ $val }}</span>
                </div>
                @endforeach
            </div>
        </div>
    ...
    {{-- ── Row 2: Application Funnel + Industry Chart ── --}}
    <div class="grid lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="chart-card section-card p-6">
            ...
            @endphp
            <div class="space-y-4">
                @foreach($funnelDefs as $key => [$label, $gradient, $textColor])
                @php $pct = round(($funnel[$key] / $funnelTotal) * 100); @endphp
                <div>
                    <div class="flex justify-between items-center text-sm mb-1.5">
                        <span class="font-semibold text-gray-700 dark:text-gray-300">{{ $label }}</span>
                        <div class="flex items-center gap-2">
                            <span class="text-xs font-black {{ $textColor }}">{{ $funnel[$key] }}</span>
                            <span class="text-xs text-gray-400">{{ $pct }}%</span>
                        </div>
                    </div>
                    <div class="h-3 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                        <div class="h-full bg-gradient-to-r {{ $gradient }} funnel-bar rounded-full shadow-sm"
                            style="width: {{ $pct }}%"></div>
                    </div>
                </div>
                @endforeach
    ...
    {{-- ── Row 3: New Users Bar + Application Growth ── --}}
    <div class="grid lg:grid-cols-2 gap-6 mb-6">
        <div class="chart-card section-card p-6">
    ...
    {{-- ── Row 4: Recent Signups + Top Challenges ── --}}
    <div class="grid lg:grid-cols-2 gap-6">
        ...
        <div class="section-card p-6">
            <div class="flex items-center justify-between mb-5">
                ...
                <a href="{{ route('admin.users.index') }}" class="text-sm text-primary-600 hover:underline font-semibold">View all →</a>
            </div>
            <div class="space-y-2">
                @foreach($recentUsers as $u)
                <div class="flex items-center gap-3 p-2.5 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 transition group">
                    <img src="{{ $u->logoUrl() }}" class="w-10 h-10 rounded-xl object-cover shadow-sm group-hover:scale-105 transition">
                    <div class="flex-1 min-w-0">
                        <p class="font-semibold text-sm truncate">{{ $u->companyName() }}</p>
                        <p class="text-[10px] text-gray-400">{{ $u->email }} · {{ $u->created_at->diffForHumans() }}</p>
                    </div>
                    <div class="flex flex-col items-end gap-1">
                        <span class="text-[10px] px-2 py-0.5 rounded-full font-bold {{ $u->isStartup() ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300' : 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300' }}">
                            {{ ucfirst($u->role) }}
                        </span>
                        <span class="text-[9px] px-1.5 py-0.5 rounded-full font-bold
                            {{ $u->status === 'approved' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300' : ($u->status === 'pending' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300') }}">
                            {{ ucfirst($u->status) }}
                        </span>
                    </div>
                </div>
        ...
        <div class="section-card p-6">
            ...
            <div class="space-y-3">
                @forelse($topChallenges as $i => $ch)
                @php $maxApps = $topChallenges->first()->applications_count ?: 1; @endphp
                <div class="flex items-center gap-3">
                    <span class="w-6 h-6 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-xs font-black text-gray-500 flex-shrink-0">{{ $i+1 }}</span>
                    <div class="flex-1 min-w-0">
                        <p class="font-semibold text-sm truncate">{{ $ch->title }}</p>
                        <div class="h-1.5 bg-gray-100 dark:bg-gray-800 rounded-full mt-1 overflow-hidden">
                            <div class="h-full bg-gradient-to-r from-amber-400 to-orange-500 rounded-full transition-all duration-700"
                                style="width: {{ round(($ch->applications_count / $maxApps) * 100) }}%"></div>
                        </div>
                    </div>
                    <span class="text-sm font-black text-amber-600 dark:text-amber-400 flex-shrink-0">{{ $ch->applications_count }}</span>
                </div>
                @empty
    ...
    {{-- ── Row 5: Live Matching Feed & Connection Logs (Phase 3 matching logger) ── --}}
            ...
            @forelse($recentMatches as $match)
                ...
                <div class="glass-card rounded-2xl p-5 border border-gray-100 dark:border-gray-800 shadow-md flex flex-col justify-between group hover:border-primary-400 transition-all duration-300">
                    <div class="flex items-center justify-between gap-4 mb-4">
                        ...
                        <span class="text-[9px] font-bold text-gray-400" title="{{ $match->matched_at }}">
                    ...
                    <div class="flex items-center justify-center gap-4 py-3">
                        <div class="text-center w-24">
                            <img src="{{ $u1->logoUrl() }}" class="w-12 h-12 rounded-xl object-cover mx-auto ring-2 ring-white dark:ring-gray-900 shadow-md bg-white">
                            <p class="text-xs font-bold truncate mt-2 text-gray-900 dark:text-white">{{ $u1->companyName() }}</p>
                        ...
                        <div class="text-center w-24">
                            <img src="{{ $u2->logoUrl() }}" class="w-12 h-12 rounded-xl object-cover mx-auto ring-2 ring-white dark:ring-gray-900 shadow-md bg-white">
                            <p class="text-xs font-bold truncate mt-2 text-gray-900 dark:text-white">{{ $u2->companyName() }}</p>
                    ...
                    <div class="border-t border-gray-100 dark:border-gray-800/60 pt-3 mt-4 flex items-center justify-between text-[9px] font-bold text-gray-400 uppercase tracking-wider">
                        ...
                        <div class="flex items-center gap-2">
                            <span class="{{ !is_null($match->rating_by_user_one) ? 'text-green-500' : 'text-gray-400' }}">
                                {{ !is_null($match->rating_by_user_one) ? '★ U1 Rated' : '☆ U1 Pending' }}
                            </span>
                            <span>•</span>
                            <span class="{{ !is_null($match->rating_by_user_two) ? 'text-green-500' : 'text-gray-400' }}">
                                {{ !is_null($match->rating_by_user_two) ? '★ U2 Rated' : '☆ U2 Pending' }}
                            </span>
                        </div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const tlLabels = {!! json_encode(collect($timeline)->pluck('date')) !!};
    ...
    new Chart(document.getElementById('statusChart'), {
        ...
        data: {
            ...
            datasets: [{
                data: [{{ $userStatus['approved'] }}, {{ $userStatus['pending'] }}, {{ $userStatus['suspended'] }}, {{ $userStatus['rejected'] }}],
    ...
    new Chart(document.getElementById('industryChart'), {
        type: 'bar',
        data: {
            labels: {!! json_encode($topIndustries->pluck('name')) !!},
            datasets: [
                { label: 'Startups',   data: {!! json_encode($topIndustries->pluck('startups'))   !!}, backgroundColor: '#6366f1', borderRadius: 6 },
                { label: 'Corporates', data: {!! json_encode($topIndustries->pluck('corporates')) !!}, backgroundColor: '#a855f7', borderRadius: 6 },
            ],
        },
        options: {
    ...
    new Chart(document.getElementById('usersChart'), {
        type: 'bar',
        data: {
            labels: {!! json_encode(collect($usersData)->pluck('date')) !!},
            datasets: [
                { label: 'Startups',   data: {!! json_encode(collect($usersData)->pluck('startups'))   !!}, backgroundColor: '#6366f1', borderRadius: 8 },
                { label: 'Corporates', data: {!! json_encode(collect($usersData)->pluck('corporates')) !!}, backgroundColor: '#a855f7', borderRadius: 8 },
            ],
        },
        options: {
            responsive: true,
    ...
    new Chart(document.getElementById('connectionsChart'), {
        type: 'line',
        data: {
            labels: {!! json_encode(collect($connectionsData)->pluck('date')) !!},
            datasets: [{
                label: 'Connections',
                data: {!! json_encode(collect($connectionsData)->pluck('connections')) !!},
                borderColor: '#f43f5e',
