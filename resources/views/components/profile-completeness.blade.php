{{--
    ...
    Usage: @include('components.profile-completeness')
--}}
@auth
@php
    $user    = auth()->user();
...
<div x-data="{ open: false }" class="glass-card rounded-[1.5rem] p-5 shadow-lg border-glow mt-6 reveal reveal-delay-2">
    ...
    <div class="flex items-center justify-between mb-3 cursor-pointer select-none group" @click="open = !open">
        <div class="flex items-center gap-3 flex-wrap">
            <div class="w-8 h-8 rounded-xl bg-gray-100 dark:bg-gray-800 flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform">
                <svg class="w-4 h-4 {{ $color['text'] }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            </div>
            <div>
                <span class="text-sm font-black text-gray-900 dark:text-white block uppercase tracking-wider">Profile Completeness</span>
                <span class="text-xs font-bold {{ $color['text'] }} mt-0.5">{{ $pct }}% · {{ $color['label'] }}</span>
            </div>
        </div>
    ...
    <div class="h-3 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden shadow-inner relative">
        <div class="h-full bg-gradient-to-r {{ $color['bar'] }} rounded-full shadow-sm relative overflow-hidden transition-all duration-1000 ease-out"
             style="width: 0%" x-intersect="$el.style.width = '{{ $pct }}%'">
    ...
    <div x-show="open" x-cloak
        ...
        <div class="flex items-center gap-3 text-sm p-2 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800/50 transition">
            <span class="flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center shadow-sm {{ $c['ok'] ? 'bg-green-100 dark:bg-green-900/30 text-green-600' : 'bg-gray-100 dark:bg-gray-800 text-gray-400' }}">
                @if($c['ok'])
            ...
            <span class="font-semibold {{ $c['ok'] ? 'line-through text-gray-400' : 'text-gray-700 dark:text-gray-300' }}">
                {{ $c['label'] }}
            </span>
            @if(!$c['ok'])
        ...
        <div class="mt-4 pt-4 border-t border-gray-100 dark:border-gray-800/50">
            ...
            <div class="flex flex-wrap gap-2">
                @foreach($missing as $tip)
                <a href="{{ $user->isStartup() ? route('startup.profile.edit') : route('corporate.profile.edit') }}"
