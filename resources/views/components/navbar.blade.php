<nav x-data="navbarApp()" x-init="init()" class="fixed top-0 left-0 right-0 z-50 transition-all duration-300" 
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex items-center gap-8">
                ...
                <div class="hidden md:flex items-center gap-2">
                    ...
                    @elseif(auth()->user()->isCorporate())
                        ...
                        <a href="{{ route('corporate.challenges.index') }}" class="nav-link {{ request()->routeIs('corporate.challenges.*') ? 'nav-link-active' : '' }}">Challenges</a>
                    ...
                    @unless(auth()->user()->isAdmin())
                        ...
                        <a href="{{ route('leaderboard') }}" class="nav-link {{ request()->routeIs('leaderboard') ? 'nav-link-active' : '' }}">
                            🏆 Ranks
                        </a>
                    @endunless
                </div>
                    ...
                    <button @click="open = !open"
                        ...
                        <div class="relative">
                            <div class="w-9 h-9 rounded-xl flex items-center justify-center text-white font-black text-sm shadow-lg transition-all duration-300 group-hover:scale-110 group-hover:rotate-3
                                @if(auth()->user()->isStartup()) bg-gradient-to-br from-violet-500 to-purple-700 shadow-violet-500/30
                                @elseif(auth()->user()->isCorporate()) bg-gradient-to-br from-blue-500 to-cyan-600 shadow-blue-500/30
                                @else bg-gradient-to-br from-amber-500 to-orange-600 shadow-amber-500/30 @endif">
                    ...
                    <div x-show="open" @click.outside="open = false" x-cloak
                        ...
                        <div class="relative overflow-hidden px-5 pt-5 pb-4">
                            ...
                            <div class="absolute inset-0 opacity-20
                                @if(auth()->user()->isStartup()) bg-gradient-to-br from-violet-600 via-purple-700 to-transparent
                                @elseif(auth()->user()->isCorporate()) bg-gradient-to-br from-blue-600 via-cyan-700 to-transparent
                                @else bg-gradient-to-br from-amber-600 via-orange-700 to-transparent @endif">
                            </div>
                            <div class="relative flex items-center gap-4">
                                ...
                                <div class="min-w-0 flex-1">
                                    <p class="font-black text-white text-base leading-tight truncate">{{ auth()->user()->companyName() }}</p>
                                    <p class="text-xs text-gray-400 font-medium truncate mt-0.5">{{ auth()->user()->email }}</p>
                                    {{-- Role badge --}}
                                    <div class="mt-2">
                                        <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-widest
                                            @if(auth()->user()->isStartup()) bg-violet-500/20 text-violet-300 border border-violet-500/30
                                            @elseif(auth()->user()->isCorporate()) bg-blue-500/20 text-blue-300 border border-blue-500/30
                        ...
                        <div class="px-2 pb-2 space-y-0.5" style="border-top: 1px solid rgba(255,255,255,0.05);">
                            @if(auth()->user()->isStartup())
                                <a href="{{ route('startup.profile.edit') }}" class="premium-menu-item group" @click="open = false">
                                ...
                                <a href="{{ route('startup.applications') }}" class="premium-menu-item group" @click="open = false">
                            ...
                            @elseif(auth()->user()->isCorporate())
                                <a href="{{ route('corporate.profile.edit') }}" class="premium-menu-item group" @click="open = false">
                                ...
                                <a href="{{ route('corporate.challenges.index') }}" class="premium-menu-item group" @click="open = false">
        ...
        <div x-data="{ mobileOpen: false }" x-show="mobileOpen" x-cloak x-transition class="md:hidden pb-4 border-t border-white/10 pt-4 space-y-1 px-2 bg-[#0d0d12]/95 backdrop-blur-xl">
            @if(auth()->user()->isStartup())
                <a href="{{ route('startup.dashboard') }}" class="mobile-nav-link">Dashboard</a>
                <a href="{{ route('startup.swipe') }}" class="mobile-nav-link">Discover</a>
                <a href="{{ route('startup.challenges') }}" class="mobile-nav-link">Challenges</a>
            @elseif(auth()->user()->isCorporate())
                <a href="{{ route('corporate.dashboard') }}" class="mobile-nav-link">Dashboard</a>
                <a href="{{ route('corporate.swipe') }}" class="mobile-nav-link">Discover</a>
                <a href="{{ route('corporate.challenges.index') }}" class="mobile-nav-link">My Challenges</a>
            @elseif(auth()->user()->isAdmin())
            ...
            @unless(auth()->user()->isAdmin())
                ...
                <a href="{{ route('search') }}" class="mobile-nav-link">Search</a>
