...
<div class="max-w-6xl mx-auto px-4 py-10 space-y-8 relative z-10">
        ...
        <div class="cover-gradient h-64 relative overflow-hidden">
            ...
            <div class="absolute top-6 right-8 bg-white/20 backdrop-blur-xl rounded-[2rem] p-4 border border-white/30 text-white text-center shadow-2xl flex items-center gap-4">
                ...
                <div class="text-left pr-2">
                    ...
                    <div class="text-sm font-bold text-white drop-shadow-sm">{{ $compatibility['label'] }}</div>
                </div>
            </div>
            @endif

            {{-- Floating stat chips for startups --}}
                ...
                <div class="flex items-end gap-6">
                    ...
                    <div class="pb-2">
                        <div class="flex flex-wrap items-center gap-3 mb-2">
                            <h1 class="text-4xl font-black font-outfit text-gray-900 dark:text-white">{{ $user->companyName() }}</h1>
                            @foreach($user->badges as $b)
                            <span title="{{ $b->description }}" class="w-8 h-8 bg-gradient-to-br from-yellow-400 to-orange-500 rounded-xl flex items-center justify-center shadow-lg border-2 border-white dark:border-gray-900" >
                        ...
                        <div class="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                            ...
                            @if($profile?->city)<span>📍 {{ $profile->city }}{{ $profile->state ? ', '.$profile->state : '' }}</span>@endif
    ...
    <div class="grid lg:grid-cols-3 gap-8 stagger-container">
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            {{-- Milestone timeline --}}
            @if($user->isStartup() && $profile?->milestones?->count())
            <div class="reveal glass-card-strong rounded-3xl p-8 overflow-hidden relative">
                ...
                <div class="relative pl-10 border-l-4 border-gray-100 dark:border-gray-800 space-y-10">
                    @foreach($profile->milestones->sortBy('milestone_date') as $ms)
                    <div class="relative timeline-item group card-lift bg-white/50 dark:bg-gray-800/50 p-6 rounded-2xl border border-gray-100 dark:border-gray-700">
                        ...
                        <div class="flex justify-between items-start gap-4 mb-2">
                            <h3 class="font-black text-lg text-gray-900 dark:text-white group-hover:text-pink-500 transition-colors">{{ $ms->title }}</h3>
                            <span class="px-3 py-1 bg-gray-100 dark:bg-gray-700 rounded-lg text-xs font-bold text-gray-500 uppercase tracking-widest whitespace-nowrap">{{ $ms->milestone_date->format('M Y') }}</span>
                        </div>
                        @if($ms->description)
                            <p class="text-gray-600 dark:text-gray-400 font-medium leading-relaxed">{{ $ms->description }}</p>
                        @endif
                    </div>
                    @endforeach
                </div>
            ...
            <div class="reveal glass-card-strong rounded-3xl p-8 border-glow">
                    ...
                    <div class="bg-gray-50/50 dark:bg-gray-800/50 rounded-2xl p-6 border border-gray-100 dark:border-gray-700 card-lift">
                        ...
                        <div class="relative w-20 h-20 mx-auto">
                            <svg class="w-full h-full transform -rotate-90">
                                ...
                                <circle cx="40" cy="40" r="36" fill="none" class="{{ $item['score'] >= ($item['max']*0.8) ? 'stroke-green-500' : 'stroke-primary-500' }} transition-all duration-1000 ease-out" 
                                        stroke-width="8" stroke-linecap="round" stroke-dasharray="226.2" 
                                        style="stroke-dashoffset: {{ 226.2 - (226.2 * ($item['max'] > 0 ? $item['score']/$item['max'] : 0)) }}"></circle>
                            </svg>
                        </div>
                    </div>
                    @endforeach
                ...
                <div class="bg-gradient-to-r from-primary-600 via-purple-600 to-pink-600 rounded-2xl p-6 text-center text-white shadow-xl relative overflow-hidden">
                    ...
                    <div class="relative z-10 flex flex-col sm:flex-row items-center justify-center gap-4">
                        ...
                        <span class="text-xl font-bold uppercase tracking-widest drop-shadow-md">{{ $compatibility['label'] }} Match</span>
                    </div>
                </div>
            </div>
            @endif
        ...
        <div class="space-y-8">
            ...
            <div class="reveal reveal-delay-1 glass-card-strong rounded-3xl p-8">
                ...
                <div class="space-y-1">
                    ...
                    @endif
                    @if($profile?->city)<div class="info-row"><span class="text-sm font-bold text-gray-500 uppercase tracking-widest">Location</span><span class="text-sm font-black">{{ $profile->city }}{{ $profile->state ? ', '.$profile->state : '' }}</span></div>@endif
