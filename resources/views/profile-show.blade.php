...
<div class="max-w-6xl mx-auto px-4 py-10 space-y-8 relative z-10" x-data="{ showAnalystReport: false }">
    @include('components.back-button', ['fallback' => route('search'), 'label' => 'Back to Search'])
    ...
    <div class="reveal glass-card-strong rounded-[2.5rem] overflow-hidden shadow-2xl border-glow relative">
        ...
        <div class="cover-gradient h-64 relative overflow-hidden bg-cover bg-center bg-no-repeat" 
             style="@if($profile && $profile->banner) background-image: url('{{ asset('storage/' . $profile->banner) }}'); @endif">
            <div class="absolute inset-0 noise opacity-20"></div>
            <div class="absolute inset-0 bg-white/10 mesh-gradient opacity-40"></div>
            
            {{-- Score badge top-right (Clickable to open AI Analyst report) --}}
            @if($compatibility)
            <div @click="showAnalystReport = true" 
                ...
                <div class="relative w-16 h-16">
                    <svg class="w-full h-full" viewBox="0 0 44 44">
                        ...
                        <circle cx="22" cy="22" r="20" fill="none" stroke="white" stroke-width="4" stroke-linecap="round" 
                                style="--target-offset: {{ 125.6 - (125.6 * $compatibility['score']) / 100 }}" class="animated-ring"></circle>
                    </svg>
                    <div class="absolute inset-0 flex items-center justify-center">
                        <span class="text-xl font-black drop-shadow-md">{{ $compatibility['score'] }}</span>
                    </div>
                </div>
                <div class="text-left pr-2">
                    ...
                    <div class="text-sm font-bold text-white drop-shadow-sm flex items-center gap-1.5">
                        {{ $compatibility['label'] }} <span class="text-xs opacity-60">Report →</span>
                    </div>
                </div>
            </div>
            @endif
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
                <div class="pb-2 flex-1 min-w-0 w-full text-center md:text-left">
                    <div class="flex flex-wrap items-center justify-center md:justify-start gap-3 mb-2">
                        <h1 class="text-4xl font-black font-outfit text-gray-900 dark:text-white truncate max-w-full animate-fade-in" title="{{ $user->companyName() }}">{{ $user->companyName() }}</h1>
                        ...
                        @endphp
                        <span title="{{ $b->name }}: {{ $b->description }}" class="w-8 h-8 bg-gradient-to-br from-yellow-400 to-orange-500 rounded-xl flex items-center justify-center shadow-lg border-2 border-white dark:border-gray-900 text-base select-none" >
                            {{ $displayIcon }}
                        </span>
                        @endforeach
                    </div>
                    <div class="flex flex-wrap items-center justify-center md:justify-start gap-x-4 gap-y-2 text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                        @if($profile?->industry)<span class="text-primary-600 dark:text-primary-400">{{ $profile->industry->name }}</span>@endif
                        @if($profile?->city)
                            @php
                                ...
                                $locationStr = (strlen($stateText) > 0 && stripos($cityText, $stateText) === false)
                                    ...
                                    : $cityText;
                            @endphp
                            <span class="flex items-center gap-1">📍 {{ $locationStr }}</span>
                        @endif
                        @if($profile?->website)<a href="{{ $profile->website }}" target="_blank" class="hover:text-primary-600 transition flex items-center gap-1">🌐 Website</a>@endif
                    </div>

                    {{-- Startup Custom Stat Chips --}}
                    @if($user->isStartup() && $profile)
                    <div class="flex flex-wrap justify-center md:justify-start gap-2 mt-3 select-none">
                        ...
                        <span class="inline-flex items-center gap-1 px-3 py-1 rounded-xl text-xs font-bold bg-violet-500/10 text-violet-600 dark:text-violet-400 border border-violet-500/20">
                            👥 {{ $profile->team_size }} {{ $profile->team_size == 1 ? 'Member' : 'Members' }}
                        </span>
                        @endif
                        ...
                        <span class="inline-flex items-center gap-1 px-3 py-1 rounded-xl text-xs font-bold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
                            💰 Raised ₹{{ number_format($profile->funding_amount / 100000, 1) }}L
                        </span>
                        @endif
                    </div>
                    @endif

                    {{-- Corporate Custom Stat Chips --}}
                    @if($user->isCorporate() && $profile)
                    <div class="flex flex-wrap justify-center md:justify-start gap-2 mt-3 select-none">
                        @if($profile->company_size)
                        <span class="inline-flex items-center gap-1 px-3 py-1 rounded-xl text-xs font-bold bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-500/20">
                            🏢 {{ ucfirst($profile->company_size) }} Size
                        </span>
                        @endif
                        @if($profile->budget_min || $profile->budget_max)
                        <span class="inline-flex items-center gap-1 px-3 py-1 rounded-xl text-xs font-bold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
                            💰 Budget: ₹{{ number_format($profile->budget_min / 100000, 1) }}L – ₹{{ number_format($profile->budget_max / 100000, 1) }}L
                        </span>
                        @endif
                    </div>
                    @endif
                ...
                <div class="flex flex-wrap gap-4 pb-2 justify-center md:justify-end">
                    ...
                    @elseif($user->isStartup() || $user->isCorporate())
    ...
    <div class="grid lg:grid-cols-3 gap-8 stagger-container">
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="reveal glass-card-strong rounded-3xl p-8">
                ...
                <div class="prose prose-lg dark:prose-invert max-w-none text-gray-600 dark:text-gray-400 font-medium leading-relaxed">
                    @if($user->isStartup())
                        {{ $profile?->elevator_pitch ?? 'No description yet.' }}
                    @else
                        {{ $profile?->about ?: ($profile?->problem_statement ?? 'No description yet.') }}
                    @endif
                </div>
            </div>

            {{-- Corporate: What we're looking for --}}
            @if($user->isCorporate() && $profile?->problem_statement)
            <div class="reveal glass-card-strong rounded-3xl p-8 border border-purple-200 dark:border-purple-800/50 bg-purple-50/50 dark:bg-purple-900/10">
                ...
                <p class="text-gray-700 dark:text-gray-300 font-medium leading-relaxed text-lg">{{ $profile->problem_statement }}</p>
            </div>
            @endif
            ...
            <div class="reveal glass-card-strong rounded-3xl p-8">
                <h2 class="text-2xl font-black font-outfit mb-6 flex items-center gap-3 text-gray-900 dark:text-white">
                    <span class="w-10 h-10 rounded-xl bg-gradient-to-br {{ $iconBg }} flex items-center justify-center text-white text-xl shadow-lg">{{ $icon }}</span>
                    {{ $tagLabel }}
                </h2>
                <div class="flex flex-wrap gap-3">
                    @foreach($tags as $tag)
                    <span class="tag-chip {{ $tagColor }} cursor-default">#{{ $tag }}</span>
                    @endforeach
                </div>
            </div>
            @endif

            {{-- ══ CORE STRATEGY: PREMIUM CHRONOLOGICAL TIMELINE ══ --}}
            <div class="reveal glass-card-strong rounded-[2.5rem] p-8 sm:p-10 overflow-hidden relative border-glow">
                ...
                {{-- Beautiful Vertical Timeline Track --}}
                <div class="relative z-10 pl-6 sm:pl-10 space-y-12">
                    ...
                    {{-- Loop through timeline growth milestones --}}
                    @foreach($timelineItems as $item)
                    <div class="relative timeline-item group">
                        
                        {{-- Timeline Dot with custom hover scaling & gradient ring --}}
                        ...
                        <div class="card-lift bg-white/40 dark:bg-zinc-900/15 backdrop-blur-xl border border-gray-100 dark:border-zinc-800/60 rounded-[2rem] p-6 hover:border-primary-500/40 dark:hover:border-primary-500/40 hover:shadow-2xl transition duration-300 relative overflow-hidden">
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4 pb-4 border-b border-gray-100 dark:border-zinc-800/40">
                                ...
                                {{-- Smart Metric Badge with gradients --}}
                                @if($item['type'] === 'mission_vision')
                                ...
                                @elseif(str_contains(strtolower($item['metric']), 'arr') || str_contains(strtolower($item['metric']), 'revenue') || str_contains(strtolower($item['metric']), 'capital') || str_contains(strtolower($item['metric']), 'budget') || str_contains(strtolower($item['metric']), 'fund') || str_contains(strtolower($item['metric']), 'raised'))
                            ...
                            {{-- Custom Content: Mission & Vision split grid card --}}
                            @if($item['type'] === 'mission_vision')
                                <div class="grid md:grid-cols-2 gap-4 mt-6">
                                    ...
                                    <div class="bg-gradient-to-br from-blue-50/40 to-indigo-50/20 dark:from-blue-950/15 dark:to-indigo-950/5 border border-blue-100/50 dark:border-blue-900/20 rounded-2xl p-5 relative overflow-hidden group/m hover:border-blue-400 dark:hover:border-blue-700/80 transition duration-300">
                                        ...
                                        <p class="text-xs font-semibold text-gray-800 dark:text-zinc-300 leading-relaxed">{{ $item['mission'] }}</p>
                                    </div>
                                    ...
                                    <div class="bg-gradient-to-br from-purple-50/40 to-pink-50/20 dark:from-purple-950/15 dark:to-pink-950/5 border border-purple-100/50 dark:border-purple-900/20 rounded-2xl p-5 relative overflow-hidden group/v hover:border-purple-400 dark:hover:border-purple-700/80 transition duration-300">
                                        ...
                                        <p class="text-xs font-semibold text-gray-800 dark:text-zinc-300 leading-relaxed">{{ $item['vision'] }}</p>
                                    </div>
                                </div>
                            @endif

                            {{-- Custom Content: Dynamic Scaling progress bars for Revenue indicators --}}
                            @if(isset($item['progress']) && ($item['type'] === 'revenue' || $item['type'] === 'inception'))
                                <div class="mt-6 pt-4 border-t border-gray-100 dark:border-zinc-800/40">
                                    <div class="flex items-center justify-between mb-2">
                                        ...
                                        <span class="text-xs font-bold text-gray-900 dark:text-white">{{ $item['progress'] }}% Growth Scale</span>
                                    </div>
                                    <div class="w-full h-2.5 bg-gray-100 dark:bg-zinc-800/50 rounded-full overflow-hidden relative">
                                        <div class="h-full rounded-full bg-gradient-to-r from-primary-500 via-indigo-500 to-emerald-500 transition-all duration-1000" style="width: {{ $item['progress'] }}%"></div>
                                    </div>
            ...
            @if($compatibility && isset($compatibility['breakdown']))
            <div class="reveal glass-card-strong rounded-3xl p-8 border-glow">
                ...
                <div class="grid sm:grid-cols-2 gap-8 mb-8">
                    @foreach($compatibility['breakdown'] as $item)
                    <div class="bg-gray-50/50 dark:bg-gray-800/50 rounded-2xl p-6 border border-gray-100 dark:border-gray-700 card-lift">
                        <div class="flex items-center justify-between mb-4">
                            <h4 class="font-bold text-gray-900 dark:text-white">{{ $item['label'] }}</h4>
                            <span class="text-xl font-black {{ $item['score'] >= ($item['max']*0.8) ? 'text-green-500' : 'text-primary-500' }}">{{ round($item['score']) }}<span class="text-sm text-gray-400">/{{ $item['max'] }}</span></span>
                        </div>
                        
                        {{-- Circular Progress for sub-items --}}
                        <div class="relative w-20 h-20 mx-auto">
                            <svg class="w-full h-full transform -rotate-90">
                                <circle cx="40" cy="40" r="34" fill="none" class="stroke-gray-200/60 dark:stroke-zinc-700/50" stroke-width="6"></circle>
                                <circle cx="40" cy="40" r="34" fill="none" class="{{ $item['score'] >= ($item['max']*0.8) ? 'stroke-green-500' : 'stroke-primary-500' }} transition-all duration-1000 ease-out" 
                                        stroke-width="6" stroke-linecap="round" stroke-dasharray="213.6" 
                                        style="stroke-dashoffset: {{ 213.6 - (213.6 * ($item['max'] > 0 ? $item['score']/$item['max'] : 0)) }}"></circle>
                            </svg>
                ...
                <div class="bg-gradient-to-r from-primary-600 via-purple-600 to-pink-600 rounded-2xl p-6 text-center text-white shadow-xl relative overflow-hidden">
                    ...
                    <div class="relative z-10 flex flex-col sm:flex-row items-center justify-center gap-4">
                        <span class="text-5xl font-black drop-shadow-md">{{ $compatibility['score'] }}<span class="text-2xl text-white/70">/100</span></span>
                        <div class="h-10 w-px bg-white/20 hidden sm:block"></div>
                        <span class="text-xl font-bold uppercase tracking-widest drop-shadow-md">{{ $compatibility['label'] }}</span>
                    </div>
                </div>
            </div>
            @endif
        ...
        <div class="space-y-8">
            ...
            <div class="reveal reveal-delay-1 glass-card-strong rounded-3xl p-8">
                ...
                <div class="space-y-1">
                    @if($user->isStartup() && $profile)
                        <div class="info-row"><span class="text-sm font-bold text-gray-500 uppercase tracking-widest">Stage</span><span class="text-sm font-black">{{ $profile->stageLabel() }}</span></div>
                        @if($profile->team_size)<div class="info-row"><span class="text-sm font-bold text-gray-500 uppercase tracking-widest">Team</span><span class="text-sm font-black">{{ $profile->team_size }} <span class="text-gray-400">ppl</span></span></div>@endif
                        @if($profile->founded_year)<div class="info-row"><span class="text-sm font-bold text-gray-500 uppercase tracking-widest">Founded</span><span class="text-sm font-black">{{ $profile->founded_year }}</span></div>@endif
                        @if($profile->funding_status)<div class="info-row"><span class="text-sm font-bold text-gray-500 uppercase tracking-widest">Funding</span><span class="text-sm font-black capitalize">{{ str_replace('_',' ',$profile->funding_status) }}</span></div>@endif
                        @if($profile->funding_amount > 0)<div class="info-row"><span class="text-sm font-bold text-gray-500 uppercase tracking-widest">Raised</span><span class="text-sm font-black text-green-500">₹{{ number_format($profile->funding_amount) }}</span></div>@endif
                    @elseif($user->isCorporate() && $profile)
                        @if($profile->company_size)<div class="info-row"><span class="text-sm font-bold text-gray-500 uppercase tracking-widest">Size</span><span class="text-sm font-black capitalize">{{ str_replace('_',' ',$profile->company_size) }}</span></div>@endif
                        @if($profile->budget_min || $profile->budget_max)<div class="info-row flex-col items-start gap-1"><span class="text-sm font-bold text-gray-500 uppercase tracking-widest">Budget</span><span class="text-lg font-black text-green-500">₹{{ number_format($profile->budget_min) }} <span class="text-sm text-gray-400 font-bold px-1">to</span> ₹{{ number_format($profile->budget_max) }}</span></div>@endif
                        @if($profile->established_year)<div class="info-row"><span class="text-sm font-bold text-gray-500 uppercase tracking-widest">Est.</span><span class="text-sm font-black">{{ $profile->established_year }}</span></div>@endif
                    @endif
                    @if($profile?->city)
                        @php
                            $cityText = $profile->city;
                            ...
                            $locationStr = (strlen($stateText) > 0 && stripos($cityText, $stateText) === false)
                                ...
                                : $cityText;
                        @endphp
                        <div class="info-row"><span class="text-sm font-bold text-gray-500 uppercase tracking-widest">Location</span><span class="text-sm font-black">{{ $locationStr }}</span></div>
                    @endif
                    @if($profile?->website)<div class="info-row"><span class="text-sm font-bold text-gray-500 uppercase tracking-widest">Website</span><a href="{{ $profile->website }}" target="_blank" class="text-sm text-primary-600 hover:text-primary-700 font-black flex items-center gap-1">Visit Site <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg></a></div>@endif
                </div>
            </div>
            ...
            <div class="reveal reveal-delay-2 glass-card-strong rounded-3xl p-8">
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    @endphp
                    <div class="text-center group cursor-default" title="{{ $badge->description }}">
                        ...
                        <p class="text-[10px] text-gray-900 dark:text-white font-bold leading-tight line-clamp-2 uppercase tracking-wider">{{ $badge->name }}</p>
                    </div>
                    @endforeach
                </div>
            </div>
    ...
    {{-- ══ EXECUTIVE INVESTMENT ANALYST POPUP MODAL ══ --}}
    @if($compatibility && isset($compatibility['analyst']))
        ...
        <div class="glass-card-strong w-full max-w-4xl max-h-[85vh] rounded-[2.5rem] overflow-hidden shadow-2xl border-glow transform transition-all duration-300 flex flex-col relative z-10 bg-white/95 dark:bg-[#0d0d12]/95"
            ...
            <div class="p-6 sm:p-8 overflow-y-auto space-y-8 flex-1 scroll-smooth">
                ...
                <div class="bg-gradient-to-r from-primary-600 via-purple-600 to-pink-600 rounded-3xl p-6 text-white shadow-xl relative overflow-hidden">
                    ...
                    <div class="relative z-10">
                        ...
                        <p class="text-base sm:text-lg font-bold leading-relaxed mt-2">
                            {{ $compatibility['analyst']['summary'] ?? 'AI analysis is being generated. Please check back shortly.' }}
                        </p>
                    </div>
                </div>
                ...
                <div class="grid md:grid-cols-2 gap-8">
                    ...
                    {{-- Left Col: Profile Match Breakdown --}}
                    <div class="space-y-6">
                        ...
                        <div class="space-y-4">
                            ...
                            @foreach($breakdownMetrics as $metric)
                            @php
                            ...
                            <div class="bg-gray-50/50 dark:bg-zinc-900/30 rounded-2xl p-4 border border-gray-100 dark:border-zinc-800/40 animate-fade-in">
                                <div class="flex items-center justify-between mb-2">
                                    <span class="text-sm font-bold text-gray-800 dark:text-zinc-300 flex items-center gap-2 select-none">
                                        <span>{{ $metric['emoji'] }}</span> {{ $metric['label'] }}
                                    </span>
                                    <span class="text-sm font-black text-primary-500">
                                        {{ $metricScore }}<span class="text-xs text-gray-400">/{{ $metricMax }}</span>
                                    </span>
                                </div>
                                <div class="w-full h-2 bg-gray-100 dark:bg-zinc-800 rounded-full overflow-hidden relative">
                                    <div class="h-full rounded-full bg-gradient-to-r from-primary-500 to-indigo-500 transition-all duration-1000" style="width: {{ $pct }}%"></div>
                                </div>
                            </div>
                            @endforeach

                            {{-- AI Strengths from Gemini --}}
                            @php $strengths = $compatibility['analyst']['strengths'] ?? []; @endphp
                            @if(!empty($strengths))
                    ...
                    {{-- Right Col: Predictions Panel & ROI --}}
                    <div class="space-y-6">
                        <h4 class="text-sm font-black uppercase tracking-widest text-gray-500 dark:text-zinc-400 select-none">
                        ...
                        <div class="grid sm:grid-cols-2 gap-4">
                            ...
                            <div class="bg-gradient-to-br from-emerald-500/10 to-teal-500/10 border border-emerald-500/20 rounded-3xl p-5 text-center shadow-lg relative overflow-hidden group">
                                ...
                                <h3 class="text-2xl font-black text-emerald-600 dark:text-emerald-400 mt-2">
                                    {{ $compatibility['analyst']['roi_estimate'] ?? $compatibility['analyst']['roi'] ?? 'N/A' }}
                                </h3>
                            ...
                            <div class="bg-gradient-to-br from-indigo-500/10 to-primary-500/10 border border-indigo-500/20 rounded-3xl p-5 text-center shadow-lg relative overflow-hidden group">
                                ...
                                <h3 class="text-2xl font-black text-indigo-600 dark:text-indigo-400 mt-2">{{ $compatibility['analyst']['confidence'] ?? 0 }}%</h3>
                            ...
                            <div class="bg-gradient-to-br from-purple-500/10 to-pink-500/10 border border-purple-500/20 rounded-3xl p-5 text-center shadow-lg relative overflow-hidden group sm:col-span-2">
                                ...
                                <div class="flex items-center justify-center gap-4 mt-2">
                                    <span class="text-3xl font-black text-purple-600 dark:text-purple-400">{{ $compatibility['analyst']['success_prob_percentage'] ?? 0 }}%</span>
                                    <div class="w-16 h-1.5 bg-gray-100 dark:bg-zinc-800 rounded-full overflow-hidden relative">
                                        <div class="h-full rounded-full bg-gradient-to-r from-purple-500 to-pink-500" style="width: {{ $compatibility['analyst']['success_prob_percentage'] ?? 0 }}%"></div>
                                    </div>
                                </div>
                            </div>
                        </div>

                        {{-- Revenue Impact & AI Score breakdown --}}
                        <div class="bg-slate-50/50 dark:bg-zinc-900/30 border border-slate-100 dark:border-zinc-800/40 rounded-3xl p-6 space-y-4">
                            <div>
                                ...
                                <p class="text-base font-bold text-gray-900 dark:text-white mt-1">
                                    💰 {{ $compatibility['analyst']['revenue_impact'] ?? 'N/A' }}
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-3 mt-2">
                                    <div class="flex-1 text-center bg-primary-50 dark:bg-primary-900/20 rounded-xl p-2">
                                        ...
                                        <p class="text-lg font-black text-primary-600 dark:text-primary-400">{{ $compatibility['analyst']['rule_score'] ?? '—' }}</p>
                                    </div>
                                    ...
                                    <div class="flex-1 text-center bg-purple-50 dark:bg-purple-900/20 rounded-xl p-2">
                                        ...
                                        <p class="text-lg font-black text-purple-600 dark:text-purple-400">{{ $compatibility['analyst']['ai_score'] ?? '—' }}</p>
                                    </div>
                                    <span class="text-gray-400 font-bold">=</span>
                                    <div class="flex-1 text-center bg-gradient-to-br from-primary-100 to-purple-100 dark:from-primary-900/30 dark:to-purple-900/30 rounded-xl p-2 border border-primary-200 dark:border-primary-800">
                                        ...
                                        <p class="text-lg font-black text-gray-900 dark:text-white">{{ $compatibility['score'] ?? 0 }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                {{-- Strategist Recommendations & Risk Analysis --}}
                <div class="grid md:grid-cols-2 gap-8">
                    {{-- Recommendations --}}
                    <div class="bg-blue-500/5 border border-blue-500/25 rounded-3xl p-6 relative overflow-hidden group">
                        ...
                        <p class="text-sm font-bold text-gray-800 dark:text-zinc-200 leading-relaxed">
                            {{ $compatibility['analyst']['recommendation'] ?? 'Complete your profile to receive a personalised recommendation.' }}
                        </p>
                    </div>
                    ...
                    <div class="bg-amber-500/5 border border-amber-500/25 rounded-3xl p-6 relative overflow-hidden group">
                        ...
                        <p class="text-sm font-bold text-gray-800 dark:text-zinc-200 leading-relaxed mb-2">
                            {{ $compatibility['analyst']['risk_analysis'] ?? 'No significant risks identified by the AI at this time.' }}
                        </p>
                    </div>
                </div>
