@if($results->isEmpty())
    <div class="reveal reveal-delay-3 glass-card-strong rounded-3xl p-16 text-center border-dashed border-2 border-gray-300 dark:border-gray-700">
        ...
        <p class="text-gray-500 dark:text-gray-400 text-base mb-8 max-w-md mx-auto">We couldn't find any {{ $targetRole }}s matching your exact criteria. Try broadening your search filters.</p>
...
@else
        ...
        <a href="{{ route('profile.show', $r) }}"
            ...
            <div class="h-24 bg-gradient-to-r {{ $bgGradient }} relative">
                ...
                <div class="absolute top-4 right-4 bg-white/20 backdrop-blur-md border border-white/30 rounded-xl px-3 py-1.5 flex items-center gap-1.5 shadow-lg">
                    ...
                    <span class="text-white font-black text-sm drop-shadow-md">{{ $score['score'] }}</span>
                </div>
                @endif
            </div>
            ...
            <div class="px-6 pb-6 -mt-8 relative z-10 flex flex-col h-[calc(100%-6rem)]">
                <div class="flex items-end gap-4 mb-4">
                    <img src="{{ $r->logoUrl() }}" alt="{{ $r->companyName() }}"
                    ...
                    <div class="flex-1 min-w-0 pt-8">
                        <h3 class="font-bold text-lg text-gray-900 dark:text-white truncate group-hover:text-primary-600 transition font-outfit">{{ $r->companyName() }}</h3>
                        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ $profile?->industry?->name ?? ucfirst($r->role) }}</p>
                    </div>
                </div>

                {{-- Badges row --}}
                <div class="flex flex-wrap gap-2 mb-4">
                    @if($r->isStartup() && $profile?->stage)
                        <span class="text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-lg {{ $profile->stageColor() }}">{{ $profile->stageLabel() }}</span>
                    @endif
                    @if($profile?->city)
                        <span class="text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400">📍 {{ $profile->city }}</span>
                    @endif
                </div>

                {{-- Pitch --}}
                <p class="text-sm text-gray-600 dark:text-gray-400 line-clamp-3 leading-relaxed mb-4 font-medium flex-grow">
                    {{ $profile?->elevator_pitch ?? $profile?->problem_statement ?? 'No description available.' }}
                </p>

                {{-- Tech tags --}}
                @php $tags = $r->isStartup() ? ($profile?->tech_tags ?? []) : ($profile?->seeking_technologies ?? []); @endphp
                @if(count($tags) > 0)
                <div class="flex flex-wrap gap-1.5 mb-5 mt-auto">
                    @foreach(array_slice($tags, 0, 3) as $tag)
                    <span class="text-[10px] font-bold px-2 py-1 rounded-lg bg-primary-50 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 border border-primary-100 dark:border-primary-800/50">#{{ $tag }}</span>
                    @endforeach
                    @if(count($tags) > 3)<span class="text-[10px] font-bold px-2 py-1 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-500">+{{ count($tags) - 3 }}</span>@endif
                </div>
                @endif
                ...
                <div class="mt-auto pt-4 border-t border-gray-100 dark:border-gray-800/50">
                    <div class="flex justify-between items-center text-xs font-bold mb-2">
                        ...
                        <span class="{{ $score['color'] === 'green' ? 'text-green-500' : ($score['color'] === 'blue' ? 'text-blue-500' : ($score['color'] === 'yellow' ? 'text-yellow-500' : 'text-red-500')) }}">
                            {{ $score['label'] }}
                        </span>
                    </div>
                    <div class="h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden shadow-inner">
                        <div class="h-full rounded-full transition-all duration-1000 ease-out relative overflow-hidden
                            {{ $score['color'] === 'green' ? 'bg-gradient-to-r from-green-400 to-emerald-500' : ($score['color'] === 'blue' ? 'bg-gradient-to-r from-blue-400 to-cyan-500' : ($score['color'] === 'yellow' ? 'bg-gradient-to-r from-yellow-400 to-orange-500' : 'bg-gradient-to-r from-red-400 to-rose-500')) }}"
    ...
    {{-- Pagination --}}
    <div class="mt-12 flex justify-center">{{ $results->withQueryString()->links() }}</div>
@endif
