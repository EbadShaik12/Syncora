                ...
                <div class="px-8 pb-8 h-[calc(100%-160px)] flex flex-col overflow-y-auto hide-scrollbar relative">
                    ...
                    <div class="text-center mb-6">
                        ...
                        <?php if($card['profile']?->city): ?>
                            <p class="text-xs font-semibold text-gray-500 mt-2 bg-gray-100 dark:bg-gray-800 inline-block px-3 py-1 rounded-full">📍 <?php echo e($card['profile']->city); ?><?php echo e($card['profile']->state ? ', '.$card['profile']->state : ''); ?></p>
                        <?php endif; ?>
                    </div>
                    ...
                    <?php if($card['user']->isStartup() && $card['profile']): ?>
                        ...
                        <div class="bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-4 mb-6">
                            <p class="text-sm text-gray-700 dark:text-gray-300 font-medium leading-relaxed italic text-center">"<?php echo e($card['profile']->elevator_pitch); ?>"</p>
                        </div>
                        <?php if($card['profile']->tech_tags && count($card['profile']->tech_tags) > 0): ?>
                            <div class="flex flex-wrap justify-center gap-1.5 mb-6">
                                <?php $__currentLoopData = array_slice($card['profile']->tech_tags, 0, 5); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tag): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <span class="text-xs font-bold px-2.5 py-1 rounded-lg bg-primary-50 dark:bg-primary-900/30 text-primary-700 dark:text-primary-400 border border-primary-100 dark:border-primary-800/50">#<?php echo e($tag); ?></span>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </div>
                        <?php endif; ?>
                    <?php elseif($card['user']->isCorporate() && $card['profile']): ?>
                        <div class="bg-purple-50 dark:bg-purple-900/20 rounded-2xl p-5 mb-6 border border-purple-100 dark:border-purple-800/50 relative">
                            ...
                            <p class="text-sm text-gray-700 dark:text-gray-300 font-medium leading-relaxed text-center mt-2"><?php echo e($card['profile']->problem_statement); ?></p>
                        </div>
                        <?php if($card['profile']->seeking_technologies && count($card['profile']->seeking_technologies) > 0): ?>
                            <div class="flex flex-wrap justify-center gap-1.5 mb-6">
                                <?php $__currentLoopData = array_slice($card['profile']->seeking_technologies, 0, 5); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tag): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <span class="text-xs font-bold px-2.5 py-1 rounded-lg bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 border border-purple-100 dark:border-purple-800/50">#<?php echo e($tag); ?></span>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </div>
                        <?php endif; ?>
                    <?php endif; ?>
...
function swipeApp() {
    return {
        ...
        cards: <?php echo json_encode($cards->map(fn($c) => ['id' => $c['user']->id]), 15, 512) ?>,
