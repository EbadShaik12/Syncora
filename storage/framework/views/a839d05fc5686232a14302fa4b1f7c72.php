<?php $__env->startSection('title', 'Search ' . ($targetRole === 'startup' ? 'Startups' : 'Corporates')); ?>
    ...
    <div class="reveal flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
        <div>
            ...
            <h1 class="text-4xl sm:text-5xl font-black font-outfit text-gray-900 dark:text-white mb-2">
                Discover <span class="text-gradient from-primary-500 to-purple-600"><?php echo e($targetRole === 'startup' ? 'Startups' : 'Corporate Partners'); ?></span>
            </h1>
            <p class="text-gray-500 dark:text-gray-400 font-medium text-lg">
                <?php echo e($results->total()); ?> result<?php echo e($results->total() !== 1 ? 's' : ''); ?> found powered by AI matching
                <?php if(request()->anyFilled(['q','industry_id','city','stage','budget_min','budget_max','tech_tag'])): ?>
                    · <a href="<?php echo e(route('search')); ?>" class="text-primary-600 font-bold hover:underline">Clear all filters</a>
                <?php endif; ?>
            </p>
        </div>
        
        <div class="flex items-center gap-2 glass-card rounded-2xl p-1.5 shadow-lg border-glow">
            <a href="<?php echo e(request()->fullUrlWithQuery(['sort' => null])); ?>"
                class="px-5 py-2.5 text-sm font-bold rounded-xl transition <?php echo e(request('sort') !== 'score' ? 'bg-gradient-to-r from-gray-800 to-gray-900 text-white shadow-md' : 'text-gray-500 hover:text-gray-800 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800'); ?>">
                Newest
            </a>
            <a href="<?php echo e(request()->fullUrlWithQuery(['sort' => 'score'])); ?>"
                class="px-5 py-2.5 text-sm font-bold rounded-xl transition <?php echo e(request('sort') === 'score' ? 'bg-gradient-to-r from-primary-600 to-purple-600 text-white shadow-lg shadow-primary-500/30' : 'text-gray-500 hover:text-gray-800 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800'); ?>">
    ...
    <div class="flex flex-col lg:flex-row gap-8">
        ...
        <aside class="reveal reveal-delay-1 lg:w-80 flex-shrink-0">
            <form method="GET" id="filter-form" @submit.prevent="updateResults()" @input.debounce.300ms="updateResults()" @change="updateResults()" class="space-y-6 sticky top-24 filter-sidebar max-h-[calc(100vh-8rem)] overflow-y-auto pr-2 pb-10">
                ...
                <input type="hidden" name="tech_tag" id="active-tech-tag" value="<?php echo e(request('tech_tag')); ?>">
                ...
                <div class="glass-card-strong rounded-3xl p-6">
                    ...
                    <select name="industry_id" class="w-full text-sm px-4 py-3.5 rounded-xl border border-gray-200 dark:border-gray-700/50 bg-white/50 dark:bg-gray-900/50 outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/30 transition text-gray-900 dark:text-white font-medium appearance-none">
                        ...
                        <?php $__currentLoopData = $industries; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ind): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($ind->id); ?>" <?php if(request('industry_id') == $ind->id): echo 'selected'; endif; ?>><?php echo e($ind->name); ?></option>
                ...
                <div class="glass-card-strong rounded-3xl p-6">
                    ...
                    <input type="text" name="city" value="<?php echo e(request('city')); ?>" placeholder="e.g. Mumbai, Bangalore…"
                ...
                <?php if($targetRole === 'startup'): ?>
                <div class="glass-card-strong rounded-3xl p-6">
                    <label class="block text-xs font-bold uppercase tracking-widest text-gray-900 dark:text-white mb-4">Startup Stage</label>
                    <div class="space-y-3">
                        <?php $__currentLoopData = ['idea' => '💡 Idea Phase', 'mvp' => '🔧 MVP Ready', 'growth' => '📈 Growth Stage', 'scale' => '🚀 Scaling Up']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $val => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <label class="flex items-center gap-3 cursor-pointer group">
                            <div class="relative flex items-center justify-center">
                                <input type="radio" name="stage" value="<?php echo e($val); ?>" <?php echo e(request('stage') === $val ? 'checked' : ''); ?>
                            ...
                            <span class="text-sm font-semibold text-gray-700 dark:text-gray-300 group-hover:text-purple-600 dark:group-hover:text-purple-400 transition"><?php echo e($label); ?></span>
                        </label>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        <?php if(request('stage')): ?>
                        <a href="<?php echo e(request()->fullUrlWithQuery(['stage' => null])); ?>" class="block mt-4 text-xs font-bold text-purple-500 hover:text-purple-600 transition uppercase tracking-wider">Clear Stage Filter</a>
                        <?php endif; ?>
                    </div>
                </div>
                <?php endif; ?>
                ...
                <div class="glass-card-strong rounded-3xl p-6">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <?php $__currentLoopData = $allTags->take(20); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tag): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <button type="button" @click="setTechTag('<?php echo e($tag); ?>')"
                            class="filter-chip"
                            :class="activeTag === '<?php echo e($tag); ?>' ? 'filter-chip-active' : 'filter-chip-inactive'">
                            #<?php echo e($tag); ?>
                ...
                <div class="flex gap-3 pt-2">
                    ...
                    <a href="<?php echo e(route('search')); ?>" class="px-5 py-3.5 text-sm font-bold text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition flex items-center justify-center shadow-sm">
                        Reset
                    </a>
                </div>
            </form>
        ...
        <div class="flex-1 min-w-0">
            ...
            <div id="search-results-container">
                <?php echo $__env->make('partials.search-results', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
            </div>
        </div>
    </div>
</div>
...
function searchPage() {
    return {
        activeTag: '<?php echo e(request('tech_tag')); ?>',
        setTechTag(tag) {
        ...
        updateResults(page = null) {
            ...
            fetch('<?php echo e(route("search")); ?>?' + params.toString())
                .then(res => res.text())
                .then(html => {
                    ...
                    params.delete('ajax');
                    const newUrl = '<?php echo e(route("search")); ?>?' + params.toString();
...
function searchSuggest() {
    return {
        ...
        async fetchSuggestions(q) {
            ...
            try {
                const res = await fetch('<?php echo e(route("search.suggestions")); ?>?q=' + encodeURIComponent(q));
...
function budgetSlider() {
    return {
        minVal: <?php echo e(request('budget_min', 0)); ?>,
        maxVal: <?php echo e(request('budget_max', 5000000)); ?>,
    };
}
</script>
<?php $__env->stopPush(); ?>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\user\Desktop\startup-corporate-platform\resources\views/search.blade.php ENDPATH**/ ?>