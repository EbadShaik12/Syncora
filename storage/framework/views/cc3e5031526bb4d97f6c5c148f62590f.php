<?php $__env->startSection('title', 'Startup Dashboard'); ?>
        ...
        <div class="relative z-10 flex flex-col md:flex-row items-center md:items-start justify-between gap-8">
            <div class="flex-1">
                ...
                <h1 class="text-4xl sm:text-5xl lg:text-6xl font-black font-outfit leading-tight mb-4 text-slate-900 dark:text-white">
                    Hi, <span class="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600 dark:from-blue-200 dark:to-indigo-200"><?php echo e(auth()->user()->name); ?></span> 👋
                </h1>
                ...
                <div class="flex flex-wrap gap-4">
                    <a href="<?php echo e(route('startup.swipe')); ?>" class="magnetic bg-slate-900 hover:bg-slate-800 dark:bg-white dark:hover:bg-slate-100 text-white dark:text-blue-700 px-8 py-4 rounded-xl font-bold shadow-md hover:scale-[1.02] transition flex items-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>
                        Discover Partners
                    </a>
                    <a href="<?php echo e(route('startup.challenges')); ?>" class="magnetic border border-slate-200 dark:border-white/30 text-slate-700 dark:text-white bg-slate-100 hover:bg-slate-200 dark:bg-white/5 dark:hover:bg-white/10 px-8 py-4 rounded-xl font-bold hover:scale-[1.02] transition flex items-center gap-2">
            ...
            <div class="hidden lg:block relative w-64 h-64">
                ...
                <img src="<?php echo e(asset('images/feature-matching.png')); ?>" alt="Matching Engine" class="w-full h-full object-cover rounded-full border-4 border-white/10 shadow-2xl relative z-10 animate-float-slow">
            </div>
        </div>
    </div>
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="reveal glass-card-strong rounded-3xl p-8">
                <div class="flex items-center justify-between mb-8 border-b border-gray-200 dark:border-gray-800 pb-4">
                    ...
                    <a href="<?php echo e(route('startup.challenges')); ?>" class="text-sm font-bold text-primary-600 hover:text-primary-500 transition px-4 py-2 bg-primary-50 dark:bg-primary-900/20 rounded-xl">View all</a>
                </div>
                
                <?php if($openChallenges->count() > 0): ?>
                    <div class="grid sm:grid-cols-2 gap-5">
                        <?php $__currentLoopData = $openChallenges; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $challenge): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <a href="<?php echo e(route('startup.challenges.apply', $challenge)); ?>" class="flex flex-col p-5 rounded-2xl border border-gray-100 dark:border-gray-800 bg-white/50 dark:bg-gray-900/50 hover:border-blue-400 dark:hover:border-blue-500 transition group card-lift relative overflow-hidden">
                                <div class="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-blue-400 to-cyan-500"></div>
                                
                                <div class="flex items-start gap-4 mb-4">
                                    <img src="<?php echo e($challenge->corporate->logoUrl()); ?>" alt="" class="w-12 h-12 rounded-xl object-cover shadow-sm border border-gray-100 dark:border-gray-800">
                                    <div class="flex-1 min-w-0">
                                        <p class="text-xs font-bold text-gray-500 uppercase tracking-wider"><?php echo e($challenge->corporate->companyName()); ?></p>
                                        <h3 class="font-bold text-gray-900 dark:text-white line-clamp-1 group-hover:text-blue-600 transition"><?php echo e($challenge->title); ?></h3>
                                    </div>
                                </div>
                                <p class="text-sm text-gray-600 dark:text-gray-400 line-clamp-2 mb-4 font-medium flex-grow"><?php echo e($challenge->description); ?></p>
                                <div class="flex items-center justify-between text-xs font-bold mt-auto pt-4 border-t border-gray-100 dark:border-gray-800">
                                    <span class="bg-blue-50 dark:bg-blue-900/30 text-blue-600 px-3 py-1.5 rounded-lg border border-blue-100 dark:border-blue-800/50"><?php echo e($challenge->industry?->name); ?></span>
                                    <span class="flex items-center gap-1 text-gray-500">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                        <?php echo e($challenge->deadline->format('M d')); ?>
            ...
            <div class="reveal glass-card-strong rounded-3xl p-8">
                <div class="flex items-center justify-between mb-8 border-b border-gray-200 dark:border-gray-800 pb-4">
                    ...
                    <span class="text-2xl font-black text-yellow-500"><?php echo e($stats['badges']); ?></span>
                </div>
                ...
                <?php if($badges->count() > 0): ?>
                    <div class="grid sm:grid-cols-2 gap-4">
                        ...
                        <div class="flex items-center gap-4 p-4 rounded-2xl bg-gradient-to-r from-yellow-50 to-orange-50 dark:from-yellow-900/10 dark:to-orange-900/10 border border-yellow-200/60 dark:border-yellow-800/30 hover:border-yellow-400 transition card-lift group">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="font-bold text-gray-900 dark:text-white truncate"><?php echo e($badge->name); ?></p>
                                <?php if($badge->description): ?>
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5 truncate"><?php echo e($badge->description); ?></p>
                                <?php endif; ?>
                                <?php if($badge->pivot?->awarded_at): ?>
                                <p class="text-[10px] text-yellow-600 dark:text-yellow-500 font-bold mt-1 uppercase tracking-wider">
                                    Earned <?php echo e(\Carbon\Carbon::parse($badge->pivot->awarded_at)->diffForHumans()); ?>
                ...
                <?php if($recentConnections->count() > 0): ?>
                    <div class="space-y-4">
                        <?php $__currentLoopData = $recentConnections; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $conn): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            <a href="<?php echo e(route('chat.show', $conn)); ?>" class="flex items-center gap-4 p-3 rounded-2xl bg-white/30 dark:bg-gray-800/30 border border-gray-100 dark:border-gray-800 hover:border-pink-400 transition group card-lift">
                                <div class="relative">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="font-bold text-gray-900 dark:text-white truncate group-hover:text-pink-600 transition"><?php echo e($other->companyName()); ?></p>
                    ...
                    <a href="<?php echo e(route('chat.index')); ?>" class="block text-center text-sm font-bold text-gray-500 hover:text-pink-600 mt-6 pt-4 border-t border-gray-200 dark:border-gray-800 transition uppercase tracking-wider">View All Matches</a>
                <?php else: ?>
                    <div class="text-center py-10 bg-gray-50/50 dark:bg-gray-900/30 rounded-2xl border border-dashed border-gray-200 dark:border-gray-800">
                        ...
                        <a href="<?php echo e(route('startup.swipe')); ?>" class="text-sm font-bold text-pink-600 hover:underline">Start Swiping</a>
                    </div>
                <?php endif; ?>
            </div>
...
<div x-data="{ open: false }" class="fixed bottom-8 right-8 z-50 flex flex-col items-end gap-3">
    <div x-show="open" x-cloak
        ...
        <a href="<?php echo e(route('leaderboard')); ?>" class="glass-card-strong px-5 py-3 rounded-2xl flex items-center gap-3 font-bold text-gray-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 hover:scale-105 transition shadow-xl">
            <span class="text-xl">🏆</span> Leaderboard
        </a>
        <a href="<?php echo e(route('startup.challenges')); ?>" class="glass-card-strong px-5 py-3 rounded-2xl flex items-center gap-3 font-bold text-gray-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 hover:scale-105 transition shadow-xl">
            <span class="text-xl">⚡</span> Browse Challenges
        </a>
        <a href="<?php echo e(route('startup.swipe')); ?>" class="glass-card-strong px-5 py-3 rounded-2xl flex items-center gap-3 font-bold text-gray-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 hover:scale-105 transition shadow-xl">
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\user\Desktop\startup-corporate-platform\resources\views\startup\dashboard.blade.php ENDPATH**/ ?>