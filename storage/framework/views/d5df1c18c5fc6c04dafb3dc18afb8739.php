...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10">
    ...
    <div class="reveal bg-white dark:bg-zinc-950 rounded-2xl p-8 md:p-12 mb-10 shadow-lg relative overflow-hidden border border-slate-200 dark:border-zinc-800">
        ...
        <div class="absolute -top-20 -right-20 w-96 h-96 bg-purple-500/5 rounded-full blur-[80px]"></div>
        <div class="absolute -bottom-16 -left-16 w-80 h-80 bg-pink-500/5 rounded-full blur-[80px]"></div>
        
        <div class="relative z-10 flex flex-col md:flex-row items-center md:items-start justify-between gap-8">
            <div class="flex-1">
                <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-xs font-bold uppercase tracking-widest text-slate-600 dark:text-purple-200 mb-6">
                ...
                <h1 class="text-4xl sm:text-5xl lg:text-6xl font-black font-outfit leading-tight mb-4 text-slate-900 dark:text-white">
                    Hi, <span class="text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-pink-600 dark:from-purple-200 dark:to-pink-200"><?php echo e(auth()->user()->name); ?></span> 👋
                </h1>
                <p class="text-slate-600 dark:text-purple-100/80 max-w-xl text-lg leading-relaxed font-medium mb-8">Your innovation hub. Discover high-growth startups, post challenges, and accelerate your R&D pipeline.</p>
                
                <div class="flex flex-wrap gap-4">
                    <a href="<?php echo e(route('corporate.swipe')); ?>" class="magnetic bg-slate-900 hover:bg-slate-800 dark:bg-white dark:hover:bg-slate-100 text-white dark:text-purple-700 px-8 py-4 rounded-xl font-bold shadow-md hover:scale-[1.02] transition flex items-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>
                        Discover Startups
                    </a>
                    <a href="<?php echo e(route('corporate.challenges.create')); ?>" class="magnetic border border-slate-200 dark:border-white/30 text-slate-700 dark:text-white bg-slate-100 hover:bg-slate-200 dark:bg-white/5 dark:hover:bg-white/10 px-8 py-4 rounded-xl font-bold hover:scale-[1.02] transition flex items-center gap-2">
        ...
        <div class="space-y-8">
            ...
            <div class="reveal glass-card-strong rounded-3xl p-8">
                <div class="flex items-center justify-between mb-6 border-b border-gray-200 dark:border-gray-800 pb-4">
                    <h2 class="text-xl font-black font-outfit text-gray-900 dark:text-white flex items-center gap-2">
                        <span class="w-8 h-8 rounded-xl bg-gradient-to-br from-yellow-400 to-orange-500 flex items-center justify-center shadow-md">
                            <svg class="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/></svg>
                        </span>
                        Earned Badges
                    </h2>
                ...
                <?php $earnedBadges = auth()->user()->badges; ?>

                <?php if($earnedBadges->count() > 0): ?>
                    <div class="space-y-3">
                        <?php $__currentLoopData = $earnedBadges; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $badge): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="flex items-center gap-4 p-3 rounded-2xl bg-gradient-to-r from-yellow-50 to-orange-50 dark:from-yellow-900/10 dark:to-orange-900/10 border border-yellow-200/60 dark:border-yellow-800/30 hover:border-yellow-400 transition card-lift">
                            <div class="w-11 h-11 rounded-xl bg-gradient-to-br from-yellow-400 to-orange-500 flex items-center justify-center shadow-lg flex-shrink-0">
                                <svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/></svg>
                            </div>
                            <div class="flex-1 min-w-0">
                                <p class="font-bold text-gray-900 dark:text-white text-sm truncate"><?php echo e($badge->name); ?></p>
                ...
                <?php else: ?>
                    <div class="text-center py-8 bg-yellow-50/50 dark:bg-yellow-900/10 rounded-2xl border border-dashed border-yellow-200 dark:border-yellow-800/30">
                        <div class="w-14 h-14 mx-auto rounded-full bg-yellow-100 dark:bg-yellow-900/30 flex items-center justify-center mb-3">
                        ...
                        <p class="text-sm font-bold text-gray-500 dark:text-gray-400">No badges yet</p>
                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">Post challenges & connect with startups to earn badges!</p>
                    </div>
                <?php endif; ?>
            </div>
